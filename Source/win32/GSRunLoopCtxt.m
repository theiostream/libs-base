...
- (BOOL) pollUntil: (int)milliseconds within: (NSArray*)contexts
{
  ...
  handleEnum = NSEnumerateMapTable( handleMap );
    
  i = 0;
  while( NSNextMapEnumeratorPair( &handleEnum, (void**)&handle, (void**)&watcher ))
    {
      ...
      if( wait_return == WAIT_OBJECT_0 + num_handles)
        {
          if( msgTarget != nil )
            {
              [msgTarget performSelector: msgSelector
                          withObject: nil];
